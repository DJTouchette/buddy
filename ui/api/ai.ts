    ...
    "/api/ai/fix-ticket": {
      POST: async (req: Request) => {
        try {
          ...
          if (!ticketKey) {
            return Response.json({ error: "Missing ticketKey" }, { status: 400 });
          }

          if (!repoPath) {
            return Response.json({ error: "Missing repoPath" }, { status: 400 });
          }

          if (!baseBranch && !existingBranch) {
            return Response.json({ error: "Missing baseBranch or existingBranch" }, { status: 400 });
          }
          ...
          if (!isEnabled) {
            return Response.json(
              { error: "AI features are not enabled. Set ai.enabled: true in ~/.buddy.yaml" },
              { status: 400 }
            );
          }
